...
export class Billboard extends gfx.Transform3
{
    ...
    projectToSky(camera: gfx.Camera, sky: gfx.Mesh): void
    {
        ...
        this.resetBillboard();
        this.position.copy(sky.position);

        // Note if you simply move the vertex to the exact point of intersection, you may end up with "z-fighting" where 
        ...
        // on to the sky. 
        for(let i=0; i < this.vertices.length; i++){
            // To accomplish this, you should use a pick ray to perform
            // an intersection test with the sky box. 
            point.set(this.vertices[i].x, this.vertices[i].y);
            ray.setPickRay(point, camera);

            offset.copy(ray.direction);
