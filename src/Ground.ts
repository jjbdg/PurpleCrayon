...
export class Ground extends gfx.Mesh
{
    ...
    public reshapeGround(screenPath: gfx.Vector2[], groundStartPoint: gfx.Vector3,  groundEndPoint: gfx.Vector3, camera: gfx.Camera): void
    {
        ...
        const ground = gfx.Vector3.subtract(groundEndPoint, groundStartPoint);
        const projNormal = gfx.Vector3.cross(gfx.Vector3.UP, ground);
        const projectionPlane = new gfx.Plane(groundStartPoint, projNormal);
        ...
        for(let i=0; i < this.vertices.length; i++){
            const projVertex = projectionPlane.project(this.vertices[i]);
            const height = this.computeH(projVertex, silhouetteCurve, projectionPlane);

            if(height != 0){
                const dist = projectionPlane.distanceTo(this.vertices[i]);
                const w = Math.max(0, 1 - Math.pow(dist/5, 2));
                
                this.vertices[i].y = (1 - w) * this.vertices[i].y + w * height;
            }
        }
